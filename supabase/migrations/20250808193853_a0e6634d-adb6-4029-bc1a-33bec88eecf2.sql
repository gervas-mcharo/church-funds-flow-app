-- Phase 4: Recreate all RLS policies without super_administrator references
...
-- Recreate department_personnel policies
...
WITH CHECK (current_user_has_admin_role() OR 
           (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = ANY(ARRAY['general_secretary'::app_role, 'pastor'::app_role]))) OR 
           (EXISTS (SELECT 1 FROM department_personnel dp WHERE dp.user_id = auth.uid() AND dp.department_id = department_personnel.department_id AND dp.role = 'head_of_department'::app_role)));
...
CREATE POLICY "Users can view department personnel" 
ON public.department_personnel 
FOR SELECT 
USING (true);

-- Recreate approval_chain policies
...
CREATE POLICY "System can create approval chain entries" 
ON public.approval_chain 
FOR INSERT 
WITH CHECK (true);
...
ON public.approval_chain 
FOR UPDATE 
USING (EXISTS (SELECT 1 FROM money_requests mr WHERE mr.id = approval_chain.money_request_id AND has_department_role(auth.uid(), mr.requesting_department_id, approval_chain.approver_role)));

CREATE POLICY "Users can view accessible approval chains" 
ON public.approval_chain 
FOR SELECT 
USING (EXISTS (SELECT 1 FROM money_requests mr WHERE mr.id = approval_chain.money_request_id AND 
...
-- Recreate money_requests policies
