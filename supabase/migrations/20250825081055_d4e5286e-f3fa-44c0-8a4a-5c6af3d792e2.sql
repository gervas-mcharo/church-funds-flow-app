-- Fix the enum casting issue in advance_approval_chain function
CREATE OR REPLACE FUNCTION public.advance_approval_chain(p_request_id uuid, p_approver_id uuid, approval_status text, p_comments text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  -- Get current approval step with table alias to avoid ambiguity
  ...
  IF next_approval IS NULL THEN
    ...
    WHERE mr.id = p_request_id;
  ELSE
    -- Update to next approval level status with correct enum values
    ...
    -- Create notification for next approver(s) - Fixed enum casting issue
    INSERT INTO public.notification_queue (user_id, request_id, notification_type, message)
    SELECT 
    ...
    -- Fixed: Use CASE to map approval_level to app_role instead of casting
    LEFT JOIN public.user_roles ur ON ur.role = (
      CASE next_approval.approval_level
        WHEN 'finance_elder' THEN 'finance_elder'::app_role
        WHEN 'general_secretary' THEN 'general_secretary'::app_role
        WHEN 'pastor' THEN 'pastor'::app_role
      END
    ) AND next_approval.approval_level IN ('finance_elder', 'general_secretary', 'pastor')
    WHERE mr.id = p_request_id
